...
public class MasterMindGameTest
{
    ...
    private static int checkIncorrectCharacters(String[] in)
    {
        int out = 0;
        for (int i = 0; i < in.length; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                if (contains(VALID_CHARACTER, in[i].charAt(j))) out++;
            }
        }
        return out;
    }
    ...
    private static void printCodes(String[] in)
    {
        for (String i: in) System.out.println(i);
    }

    private static int checkMatches(String[] in)
    {
        int out = 0;
